...
export default function VolunteerPage() {
  ...
  const [sidePanel, setSidePanel] = useState('dashboard'); 
          ...
          <div style={{ display: 'flex', borderBottom: '1px solid var(--border-color)' }}>
           {['dashboard', 'forum'].map(p => (
              <button key={p} onClick={() => setSidePanel(p)} style={{
                ...
                {p === 'dashboard' ? '📊 My Stats' : '💬 Local Forum'}
              </button>
            ))}
          </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
            {sidePanel === 'forum' ? (
              <div style={{ height: '100%', margin: '-20px' }}>
                 <ForumPanel userLocation={location} userRole="volunteer" userName={profile?.name || "Demo Volunteer"} />
              </div>
            ) : (
              <>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
                  ...
                  <div>
                    <p style={{ fontWeight: 600 }}>Volunteer Profile</p>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 2 }}>
                       <span style={{ fontSize: 11, background: tier.color, color: '#fff', padding: '2px 6px', borderRadius: '4px', fontWeight: 'bold' }}>{tier.name}</span>
                       <span style={{ fontSize: 12, color: 'var(--fg-muted)' }}>{profile?.is_verified ? '✓ Verified' : 'Unverified'}</span>
                    </div>
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 20 }}>
                  {[
                    ...
                    <div key={i} style={{ background: 'var(--bg-raised)', borderRadius: 'var(--r-md)', padding: '12px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 5, color: 'var(--fg-muted)', marginBottom: 4 }}>{s.icon} <span style={{ fontSize: 11 }}>{s.label}</span></div>
                ...
                <div style={{ marginBottom: 20, padding: '12px', background: 'var(--bg-raised)', borderRadius: 'var(--r-md)' }}>
                  ...
                  <p style={{ fontSize: 10, color: 'var(--fg-muted)', marginTop: 6, textAlign: 'center' }}>Keep responding to emergencies to level up!</p>
                </div>
